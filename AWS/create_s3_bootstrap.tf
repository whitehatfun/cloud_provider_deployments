...
resource "aws_s3_bucket" "bootstrap_bucket_fw1" {
  bucket        = "${join("", tolist([var.bootstrap_s3bucket1_create, "-", random_string.randomstring.result]))}" 
  force_destroy = true
}

resource "aws_s3_bucket" "bootstrap_bucket_fw2" {
  bucket        = "${join("", tolist([var.bootstrap_s3bucket2_create, "-", random_string.randomstring.result]))}"
  force_destroy = true
}
...
#************************************************************************************
resource "aws_s3_object" "bootstrap_xml2" {
...
resource "aws_s3_object" "init-cft_txt2" {
  bucket = "${aws_s3_bucket.bootstrap_bucket_fw2.id}"
  acl    = "private"
  key    = "config/init-cfg.txt"
...
resource "aws_s3_object" "software2" {
  bucket = "${aws_s3_bucket.bootstrap_bucket_fw2.id}"
  acl    = "private"
  key    = "software/"
  source = "/dev/null"
...
resource "aws_s3_object" "license2" {
  bucket = "${aws_s3_bucket.bootstrap_bucket_fw2.id}"
  acl    = "private"
  key    = "license/authcodes"
...
resource "aws_s3_object" "content2" {
  bucket = "${aws_s3_bucket.bootstrap_bucket_fw2.id}"
  acl    = "private"
  key    = "content/"
  source = "/dev/null"
